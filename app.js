...
function renderMessages(messages) {
  ...
  for (const msg of messages) {
    ...
    `;
    li.addEventListener('click', () => toggleMessage(msg.id, li));
    el.messageList.appendChild(li);

    if (state.expandedId === msg.id) {
      renderExpanded(msg.id, li);
    }
  }
}

async function toggleMessage(id, rowEl) {
  ...
  state.expandedId = id;
  await renderExpanded(id, rowEl);
  fetchMessages();
}

async function renderExpanded(id, rowEl) {
  const existing = rowEl.nextElementSibling;
  if (existing && existing.classList.contains('message-detail')) existing.remove();
  ...
  try {
    ...
    detail.innerHTML = `<div class="msg-body">${bodyHtml}</div>`;
  } catch (err) {
    detail.innerHTML = `<p class="muted">Couldn't load this message.</p>`;
  }
}
...
window.addEventListener('beforeinstallprompt', (event) => {
  ...
  if (localStorage.getItem('venuxmail_install_dismissed') !== '1') {
...
dismissInstall?.addEventListener('click', () => {
  installBanner.hidden = true;
  localStorage.setItem('venuxmail_install_dismissed', '1');
});
